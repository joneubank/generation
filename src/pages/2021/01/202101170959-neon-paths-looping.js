  ...
  const draw = (width, height) => {
    ...
    const drawPathLayer = (color, distortion = (vec) => vec) => {
      ...
      const findNodeNeighbours = (node) => {
        const x = node.x;
        const y = node.y;
        return nodeQueue.filter((i) => {
          return i.x >= x - 1 && i.x <= x + 1 && i.y >= y - 1 && i.y <= y + 1;
        });
      };

      const findNodeLoops = (node) => {
        const x = node.x;
        const y = node.y;
        ...
        if (x > 0 && x < gridWidth - 1 && y > 0 && y < gridHeight - 1) {
          ...
          return nodeQueue.filter(
            (i) =>
              (Math.abs(x - i.x) === gridWidth - 1 && y === i.y) ||
              (Math.abs(y - i.y) === gridHeight - 1 && x === i.x),
          );
        }
      };
      ...
      const createPath = () => {
        ...
        while (
          count < countMax &&
          (pathLength < maxPathLength || maxPathLength < 0)
        ) {
          count += 1;
          ...
          const loops = findNodeLoops(current);
          ...
          const addLoop = () => {
            const connection = rng.chooseOne(loops);
            if (connection.x === current.x) {
              // x the same
              if (connection.y > current.y) {
                // current y - 0, so draw line out the left
                path.push(Vec2(current.x, -1));
                paths.push(path);
                path = [];
                path.push(Vec2(current.x, gridHeight));
              } else {
                path.push(Vec2(current.x, gridHeight));
                paths.push(path);
                path = [];
              ...
              // y the same
              if (connection.x > current.x) {
                // current x = 0, so draw line out the top
                path.push(Vec2(-1, current.y));
                paths.push(path);
                path = [];
                path.push(Vec2(gridWidth, current.y));
              } else {
                path.push(Vec2(gridWidth, current.y));
                paths.push(path);
                path = [];
          ...
          if (neighbours.length === 0 && loops.length === 0) {
            // if no unused adjacent, end path (return)
            paths.push(path);
            return;
          } else if (neighbours.length > 0 && loops.length === 0) {
            // else add an adjacent node at random
            addNeighbour();
          } else if (neighbours.length === 0 && loops.length > 0) {
            addLoop();
          } else {
            const chooseNeighbour = rng.bool(
              neighbours.length / (loops.length + neighbours.length),
            );
            if (chooseNeighbour) {
              addNeighbour();
            } else {
