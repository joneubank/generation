...
const draw = ({ context, pallete, rng, canvas, params }) => {
  ...
  const draw = (width, height) => {
    ...
    const maxCanvasRadius = (maxDim / 2) * (1 - buffer);

    const sphereData = array(0, spheres).map(() => {
      ...
      const posRadius = clamp(
        rng.float(0, rng.float() * maxCanvasRadius, (x) => Math.pow(x, 1)),
        { max: maxCanvasRadius },
      );
      ...
      const y = pos.y;
      const radius =
        maxRadius * Math.pow(1 - posRadius / maxCanvasRadius, 3) * maxDim;
    ...
    repeat(layers, (layer) => {
      ...
      Layers.combine(
        solidLayer,
        maskLayer,
        positive ? 'destination-in' : 'destination-out',
      );
...
export default () => (
  <Sketch
    ...
    controls={[
      ...
      { key: 'buffer', type: 'range', min: -0.5, max: 0.5, step: 0.01 },
      { key: 'positive', type: 'boolean' },

      { type: 'header', title: 'Spheres' },
      { key: 'spheres', type: 'range', min: 1, max: 100, step: 1 },
      { key: 'minRadius', type: 'range', min: 0, max: 1, step: 0.025 },
      { key: 'maxRadius', type: 'range', min: 0, max: 1, step: 0.025 },
    ]}
  />
);
