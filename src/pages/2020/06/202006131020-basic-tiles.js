...
const draw = ({ context, pallete, rng, canvas, params }) => {
  ...
  const draw = (width, height) => {
    ...
    const drawBlock = (pos) => {
      const blockWidth = ((width * scale) / rows) * 1.002;
      ...
      const backColor = rng.chooseOne(nicePallete);
      const frontColor = rng.chooseOne(nicePallete);
      ...
      const frontShapes = [SHAPE_QUARTER, SHAPE_SEMI, SHAPE_DIAGONAL];
    ...
    const topLeft = Vec2((-width * scale) / 2, (-height * scale) / 2);
