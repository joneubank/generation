...
export const sketch = ({ context, rng, pallete, meta, canvas }) => {
  ...
  const draw = (width, height) => {
    ...
    const fillColor = pallete.colors[2].value().darken(10).toRgbString();
    ...
    rect({
      ...
      width: width,
      height: height,
      fill: backgroundFill.value().darken(0).toRgbString(),
    });
    ...
    const circles = [];
    repeat(arms, arm=>repeat(count, i=>{

      const pos = polarToVec2((i/count+arm/arms)*2*Math.PI+angleShift, i/count*width*scale);
    ...
    const {rect: fillRect, path:fillPath, circle: fillCircle} = Draw(fillContext);
